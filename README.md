...
- How do device, traffic source, Price_tier, and channel affect the buying decision - and is that effect real, or just random noise?
...
- Re-tested the same 4 variables at each gate using two-proportion z-tests and multivariate regression, to avoid drawing conclusions from random coincidence
...
- High-priced items are easy to add to cart but checkout ( -3.55pp, p=0.006): Add decision-support tools for the high-price segment (comparisons, reviews, warranty/installment info) right after ATC.
...
- Direct and email traffic are consistently strong at both gates - affiliate/organic bring high traffic but convert poorly: Measure marketing ROI by actual conversion rate, not traffic volume. Prioritize budget for Direct and Email, and re-evaluate affiliate/organic spend.
...
├── User_Behavior_Analytics_at_DAJE_Company.pdf
├── SQL/project
